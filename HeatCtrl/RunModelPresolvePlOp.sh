...
DATA=$2
   ...
   function RunInstance {
  	echo "Running " $1 " on Heat Equation with Actuator Placement and Operation and the following data settings:" $2
       ...
       hostname >  output/$1-$2-presolved-PlOp.out
       uptime   >> output/$1-$2-presolved-PlOp.out
       ampl -R HeatCtrlPlOp.mod Instances/$2.dat PresolvePlOp.ampl $1.ampl solve.ampl BuildSolPlOp.ampl DisplayTime.ampl SaveControlsPlOp.ampl PlotPlOp.ampl >> output/$1-$2-presolved-PlOp.out
...
# nohup ./RunModelPresolvePlOp.sh ipopt RunModelPresolvePlOp.txt > output/ipopt.out 2> output/ipopt.err < /dev/null &
