...
#######################################################################
#
# Goal is to place and operate actuators to drive temperature to a 
# desired state.
# Model is adapted from Hante 2015 and Iftime, Demetriou 2009
# 
# Model Characterization
# ~~~~~~~~~~~~~~~~~~~~~~
# Type of PDE:         Heat equation with Dirichlet boundary
# Class of Integers:   mesh-independent time-dependent control
# Type of Constraints: SOS-1 constraint on controls at every time step
#		       Big-M constraint on continous control
# Discretization:      Five-point finite difference stencil
#			with trapezoidal/implicit euler
# 
# Problem Characteristics:
# Objective    -  quadratic
# Constaints   -  linear equations
# Binary Vars  -  space mesh-independent, time mesh-dependent
#
#######################################################################
#
# AMPL coding 
...
var v{T cross locations};			# ... continous control
var w{T cross locations}, binary;		# ... binary control

# Solution of Presolve

param ul{TxIxJ cross locations}, default 0;	# ... inhomogenous solution part
...
param Instance, symbolic;

# Information for Rounding on fine Meshes
param FracControl, symbolic, default "";
param Twfrac;
param wfrac{0..Twfrac cross locations};
param vfrac{0..Twfrac cross locations};
...
minimize PresolvedObjective: 
		hx*hy* sum{(i,j) in IxJ: i>0 && i<N && j>0 && j<M} (uh[Tn,i,j]
			+sum{(t,l) in TxL:t>0} (v[t,l]*ul[Tn-t+1,i,j,l]) -udesired[i,j])^2
	   + 2*hx*hy*ht*( sum{(k,i,j) in TxIxJ: i>0 && i<N && j>0 && j<M && k>0 && k<Tn} (uh[k,i,j]
			+sum{(t,l) in TxL:t>0 && t<=k} (v[t,l]*ul[k-t+1,i,j,l]) )^2
			 +sum{(i,j) in IxJ: i>0 && i<N && j>0 && j<M} (uh[0,i,j]^2
				+(uh[Tn,i,j]+sum{(t,l) in TxL:t>0} (v[t,l]*ul[Tn-t+1,i,j,l]))^2)/2)
   + ht/500 *   ( sum{(k,l) in TxL: k>0 && k<Tn} (v[k,l]^2)
			 +sum{l in locations}(0^2+v[Tn,l]^2)/2      );
